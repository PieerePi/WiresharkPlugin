-- Dump RTP PS payload(96) to ps file
...
do
    ...
    local fp_payload = ProtoField.bytes("ps.payload", "Raw")
    ...
    function proto_ps.dissector(tvb, pinfo, tree)
        ...
        pinfo.columns.protocol = "Simple PS(96)"
    end
    ...
    function proto_ps.init()
        payload_type_table:add(96, proto_ps)
    end
                ...
                for id,stream in pairs(stream_infos) do
                    if stream and stream.file then
                        ...
                        local anony_fuc = function ()
                            twappend("ffplay -x 720 -autoexit "..stream.filename)
                            --copy_to_clipboard("ffplay -x 720 -autoexit "..stream.filepath)
                            os.execute(ffmpeg_path.."ffplay -x 720 -autoexit "..stream.filepath)
                        end
    ...
    register_menu("Video/Export Simple PS(96)", dialog_default, MENU_TOOLS_UNSORTED)
end
